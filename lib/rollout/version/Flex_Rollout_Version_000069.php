	...
	public function rollout()
	{
		...
		$strSQL = "	CREATE TABLE account_history
					(
						...
						creation_timestamp TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'time at which the change in the account record was made',
						...
						disable_ddr TINYINT(1) NOT NULL COMMENT 'boolean flagging the disabling of the admin fee. 0 = charge lpf, 1 = don''t charge lpf',
		...
		$strSQL = "	INSERT INTO account_history (creation_timestamp, account_id, billing_type, billing_method, disable_ddr, late_payment_amnesty, tio_reference_number)
					SELECT NOW(), Id, BillingType, BillingMethod, DisableDDR, LatePaymentAmnesty, tio_reference_number
					FROM Account;";
		$result = $dbAdmin->query($strSQL);
		if (PEAR::isError($result))
		{
