...
class Flex_Rollout_Version_000008 extends Flex_Rollout_Version
{
	...
	public function rollout()
	{
		...
		{
			throw new Exception(__CLASS__ . ' Failed to create provisioning_type table. ' . $qryQuery->Error());
		}
		...
		if (!$qryQuery->Execute($strSQL))
		{
			throw new Exception(__CLASS__ . ' Failed to create carrier_module_provisioning_support table. ' . $qryQuery->Error());
		}
		...
		if (!$qryQuery->Execute($strSQL))
		{
			throw new Exception(__CLASS__ . ' Failed to create the active_statuses table. ' . $qryQuery->Error());
		}
		...
		if (!$qryQuery->Execute($strSQL))
		{
			throw new Exception(__CLASS__ . ' Failed to insert default data into the active_statuses table. ' . $qryQuery->Error());
		}
	}
