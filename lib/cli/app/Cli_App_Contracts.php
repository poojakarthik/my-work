...
class Cli_App_Contracts extends Cli
{
	...
	private function _updateContracts($strEffectiveDate=null, $iServiceId=null)
	{
		...
		while ($arrContractService = $oResult->fetch_assoc()) {
			...
			Log::getLog()->log('Line '.Constant_Group::getContantGroup('service_line_status')->getConstantName($arrContractService['LineStatus']).' @ '.date('Y-m-d', $intLineStatusDate).'; ', false);
