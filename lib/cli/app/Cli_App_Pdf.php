	...
	function run()
	{
		...
		try
		{
			...
			else
			{
				...
				$instanceRef = 'i'.number_format(ceil(microtime(true)*1000000), 0, '.', '');	// number_format(), because strval seems to be using scientific notation sometimes
			...
			foreach ($arrFiles as $strSource => $strDestination)
			{
				...
				$this->log("Memory usage before forced resource release: " . self::getMemoryUsageFriendly());
				$this->startErrorCatching();
				$pdfTemplate->destroy();
				$this->dieIfErred();
				$this->log("Memory usage after forced resource release:  " . self::getMemoryUsageFriendly());

				// Save the document to file
				$this->log("Memory usage before saving PDF to file:      " . self::getMemoryUsageFriendly());
				$this->startErrorCatching();
				...
				unset($pdf);
				$this->dieIfErred();
				$this->log("Memory usage after saving PDF to file:       " . self::getMemoryUsageFriendly());
