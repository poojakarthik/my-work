		...
		else
		{
			switch ($strStatement)
			{
				...
				case 'selPreferredMimeType':
					$arrPreparedStatements[$strStatement]	= new StatementSelect("file_type_mime_type ftmt JOIN mime_type mt ON ftmt.mime_type_id = mt.id", "mt.*", "ftmt.file_type = <id>", "is_preferred_mime_type DESC, ftmt.id DESC", 1);
					break;
