 ...
 * DO_SalesValidation
 *
 * Used to encapsulate all the high-level input validation rules that are used in the various DO_Sales model classes
 * 
 * This class should probably just be called Validation, and located in the classes root dir, but it is used by both the Flex project and the Sales project
 * so I wanted to keep it in the do directory
 *
 * NOTE: These validation methods do not concern themselves with whether or not the value in question is manditory.  That must be done at a higher level.
 *			That is to say that each function assumes the parameter passed is an actual value, not null.  The value passed should never be null.
 *
 * @class	DO_SalesValidation
 * @abstract
 */
abstract class DO_SalesValidation
{
