...
class Invoice extends ORM
{
	...
	private function _addPlanCharges($arrPlanDetails, $arrServices, $intPrimaryService=NULL)
	{
		...
		if ($strEarliestCDR)
		{
			...
			{
				$rResult	= $qryQuery->Execute("SELECT * FROM Charge WHERE ChargeType = 'PCAD' AND Account = {$this->Account} AND (Service IN ({$strServiceIds}) OR ({$arrPlanDetails['Shared']} = 1 AND Service IS NULL)) AND Status = ".CHARGE_INVOICED." AND invoice_run_id != {$this->_objInvoiceRun->invoice_run_id} LIMIT 1");
				if ($rResult === false)
				{
					throw new Exception("DB ERROR: ".$qryQuery->Error());
				}
