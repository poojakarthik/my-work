<?php
class Correspondence_Run_Dispatch extends ORM_Cached
{
	protected 			$_strTableName			= "correspondence_run_dispatch";
	protected static	$_strStaticTableName	= "correspondence_run_dispatch";
	...
	public static function  getForRunId($iRunId)
	{
		...
		$sSql	= "	SELECT 	*
					...
					WHERE 	correspondence_run_id = {$iRunId}";
		$oQuery		= new Query();
	...
	public static function getForRunAndTemplateCarrierModule($iRunId, $iTemplateCarrierModuleId)
	{
		...
		$sSql	= "	SELECT 	*
					FROM 	".self::$_strStaticTableName."
					WHERE 	correspondence_template_carrier_module_id = {$iTemplateCarrierModuleId}
					AND 	correspondence_run_id = {$iRunId}";
		$oQuery		= new Query();
		$mResult	= $oQuery->Execute($sSql);
		$aRow 		= $mResult->fetch_assoc();
		return ($aRow['id'] == null ? new self() : new self ($aRow));
	}
