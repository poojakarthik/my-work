...
class Correspondence_Dispatcher_YellowBillingCSV extends Correspondence_Dispatcher_File
{
			...
			if (in_array($iDeliveryMethod, $this->_aDeliveryMethods)) {
				...
				foreach ($aCorrespondence as $oCorrespondence) {
					...
					if ($this->_bPreprinted) {
						...
						if (false === @copy($oCorrespondence->pdf_file_path, $sTempPdfName)) {
	...
	protected function _configureFileExporter($aColumns) {
		...
		foreach($aColumns as $aColumn) {
			...
			if ($aColumn['length'] != null && $aColumn['pad']) {
				$oField->setMinimumLength($aColumn['length'])->setMaximumLength($aColumn['length'])->setPaddingString($aColumn['pad']['string'])->setPaddingStyle($aColumn['pad']['style']);
			}
			if ($aColumn['default'] != null) {
				$oField->setDefaultValue($aColumn['default']);
			}
