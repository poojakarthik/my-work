 ...
 class Invoice_Export_XML
 {
	 ...
	 */
 	static public function export($arrInvoice, $bolDebug = FALSE)
 	{
		...
		self::_addElement($xmlStatement, 'OutstandingBalance', number_format($arrInvoice['AccountBalance'], 2 + $arrInvoice['adjustment_total'] + $arrInvoice['adjustment_tax'], 2, '.', ''));
		self::_addElement($xmlStatement, 'OverdueBalance', number_format($arrCustomer['OverdueBalance'], 2 + $arrInvoice['adjustment_total'] + $arrInvoice['adjustment_tax'], 2, '.', ''));
