<?php
/**
 * Collection_Severity_Warning
 *
 * This is an example of a class that extends ORM_Cached
 *
 * @class	Collection_Severity_Warning
 */
class Collection_Severity_Warning extends ORM_Cached
{
	protected 			$_strTableName			= "collection_severity_warning";
	protected static	$_strStaticTableName	= "collection_severity_warning";
	
	public function getWarning()
	{
		return Collection_Warning::getForId($this->collection_warning_id);
	}
	...
	public static function removeForSeverityId($iSeverityId)
	{
		...
		if ($mResult === false)
		{
			throw new Exception_Database("Failed to turn off fk's prior to removing warnings for severity. ".$oQuery->Error());
		}
		
		// Delete the records
		$mResult = $oQuery->Execute("	DELETE FROM collection_severity_warning
		...
		if ($mResult === false)
		{
			...
			throw new Exception_Database("Failed to remove collection_severity_warning records. ".$oQuery->Error());
		}
