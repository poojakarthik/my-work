...
require_once dirname(__FILE__) . "/../Flex_Barcode.php";

class Flex_Barcode_CODE128 extends Flex_Barcode
{
	private $_type = 'code128';
	private $_barcodeheight = 30;
	private $_font = 2;  
	private $_barwidth = 1;
	private $code;
	private $_includeLabel = FALSE;
	private $_quietZone = 0;
	...
	function draw($text, $imgtype='png')
	{
		...
		$barcodewidth = 2 * $this->_quietZone;
		...
		// calculated by taking the bar height (plus the font 
		// height if a label is being added).
		...
		{
			$barcodelongheight = (int) $this->_barcodeheight;
		}
		...
		$img = imagecreate($barcodewidth, $barcodelongheight + ($this->_includeLabel ? (imagefontheight($this->_font) + 1) : 0));
		...
		// First, print the label, centered across the bottom.
		if ($this->_includeLabel)
		{
			imagestring(
				$img,
	...
	/**
	* @internal
	* In the Flex_Barcode_CODE128 constructor, we initialize
	...
	*/
	...
	function setQuietZone($pxWidth)
	{
		$this->_quietZone = intval($pxWidth);
	}

	function setBarWidth($pxWidth)
	{
		$this->_barwidth = intval($pxWidth);
	}

	function setBarCodeHeight($pxHeight)
	{
		$this->_barcodeheight = intval($pxHeight);
	}

	function setFontSize($fontSize)
	{
		$this->_font = min(max(intval($fontSize), 1), 5);
	}

	function setIncludeLabel($bolIncludeLabel)
	{
		$_includeLabel = ($bolIncludeLabel === TRUE);
	}
