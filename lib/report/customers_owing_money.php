...
$selAccounts		= new StatementSelect(	"((Account LEFT JOIN Contant ON Account.PrimaryContact = Contact.Id) LEFT JOIN Invoice ON Account.Id = Invoice.Account) LEFT JOIN CustomerGroup ON Account.CustomerGroup = CustomerGroup.Id",
											"Account.Id AS 'Account #', Account.BusinessName AS 'Business Name', CONCAT(Contact.FirstName, ' ', Contact.LastName) AS 'Contact Name', LPAD(Contact.Phone, 10, '0') AS 'Contact Phone'",
											"Account.Archived IN (".ACCOUNT_ACTIVE.", ".ACCOUNT_CLOSED.") AND Invoice.Status IN (".INVOICE_COMMITTED.", ".INVOICE_DISPUTED.")",
											"Invoice.Account",
											NULL,
											"Invoice.Account \n HAVING 'Total Overdue' != 0.0");
