...
abstract class NormalisationModule
{
	...
	function __construct($errErrorHandler=NULL, $rptNormalisationReport=NULL)
	{
		...
		$this->_selFindOwner			= new StatementSelect("Service JOIN Account ON Account.Id = Service.Account", "Service.*", "FNN = <fnn> AND ((CAST(<date> AS DATE) BETWEEN Service.CreatedOn AND Service.ClosedOn AND Status = ".SERVICE_DISCONNECTED.") OR ISNULL(Service.ClosedOn)) AND Account.Archived IN ($strAccountStatus)", "Service.CreatedOn DESC, Account DESC", "1");
		$this->_selFindOwnerIndial100	= new StatementSelect("Service JOIN Account ON Account.Id = Service.Account", "Service.*", "FNN LIKE <fnn> AND (Indial100 = TRUE) AND ((CAST(<date> AS DATE) BETWEEN Service.CreatedOn AND Service.ClosedOn AND Service.Status = ".SERVICE_DISCONNECTED.") OR ISNULL(Service.ClosedOn)) AND Account.Archived IN ($strAccountStatus)", "Service.CreatedOn DESC, Account DESC", "1");
		
		$this->_selFindOwnerNow 			= new StatementSelect("Service JOIN Account ON Account.Id = Service.Account", "Service.*", "FNN = <fnn> AND Service.Status != ".SERVICE_ARCHIVED." AND Account.Archived IN ($strAccountStatus)", "ISNULL(Service.ClosedOn) DESC, Service.ClosedOn DESC, Account DESC", "1");
		$this->_selFindOwnerNowIndial100	= new StatementSelect("Service JOIN Account ON Account.Id = Service.Account", "Service.*", "(FNN LIKE <fnn>) AND (Indial100 = TRUE) AND Service.Status != ".SERVICE_ARCHIVED." AND Account.Archived IN ($strAccountStatus)", "ISNULL(Service.ClosedOn) DESC, Service.ClosedOn DESC, Account DESC", "1");
