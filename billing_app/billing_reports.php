...
$GLOBALS['appBilling'] = new ApplicationBilling(Array());

Debug("Grabbing Preliminary Data...");

// Get Profit Data
$selProfitData = new StatementSelect("InvoiceRun", "*", "BillingDate < <BillingDate>", "BillingDate DESC", 1);
$arrProfitData['ThisMonth']	= $GLOBALS['appBilling']->CalculateProfitData('46afb6cf2f619');
 ...
 * @method
 */
function InitExcelFormats($wkbWorkbook)
{		
	$arrFormat = Array();
...
// CreateReportRatePlan
function CreateReportRatePlan($arrProfitData)
{	
	// Statements
	$arrCols = Array();
	...
	foreach ($arrServiceTypes as $intServiceType=>$arrRatePlans)
	{
		...
		$strFilename = "/home/richdavis/Desktop/Plan_Summary_with_Breakdown_($strServiceType).xls";
		@unlink($strFilename);
		$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
		
		// Init formats
		$arrFormat = InitExcelFormats($wkbWorkbook);
		
		foreach ($arrRatePlans as $arrRatePlan)
		{
			// Debug Info
			$intI++;
			Debug("Summarising Plan $intI of $intPlanCount ({$arrRatePlan['Description']})");
			...
			$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Plan Summary for Telco Blue");
			...
			$wksWorksheet->writeString(4, 9, $arrProfitData['ThisMonth']['InvoiceRun']);
			...
			while ($arrCallType = $selCallTypes->Fetch())
			{
				$intRow++;
				$i = $intRow + 1;
				Debug("\t\tGetting Call Data for {$arrCallType['Description']}...");
...
function CreateReportAdjustmentSummary($arrProfitData)
{
	$arrCols = Array();
	...
	//$arrCols['MeanCharge']			= "AVG(Charge.Amount)";
	$arrCols['TotalCharge']			= "SUM(Charge.Amount)";
	//$selTypeSummaries = new StatementSelect("Charge LEFT JOIN Employee ON Employee.Id = Charge.CreatedBy", $arrCols, "Charge.InvoiceRun = <InvoiceRun> AND Employee.Id IS NOT NULL", "Charge.Description", NULL, "Charge.Description");
	...
	$strFilename = "/home/richdavis/Desktop/Adjustment_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Adjustment Summary for Telco Blue");
	...
	Debug("Writing Adjustments Summary...");
	...
	Debug("Writing Adjustment Type Breakdowns...");
	...
	{
		Debug("\tWriting Summary for $strNature...");
...
function CreateReportServiceSummary($arrProfitData)
{
	...
	$strFilename = "/home/richdavis/Desktop/Service_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Service Summary for Telco Blue");
...
function CreateReportCustomerSummary($arrProfitData)
{
	...
	$strFilename = "/home/richdavis/Desktop/Customer_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Customer Summary for Telco Blue");
	$wksWorksheet->writeString(0, 1, $strPageTitle, $arrFormat['PageTitle']);
	...
	//$wksWorksheet->writeString(11, 0, "Customers Re-Opened"					, $arrFormat['TextBold']);
	...
	foreach ($arrProfitData as $arrData)
	{
		Debug("Generating Data for {$arrData['InvoiceRun']}...");
		...
		//$wksWorksheet->writeNumber(11, $intCol, $intCustomersReOpened				, $arrFormat['Integer']);
...
function CreateReportRecurringAdjustments($arrProfitData)
{
	...
	//$selChargeRemaining	= new StatementSelect("RecurringCharge JOIN Account ON Account.Id = RecurringCharge.Account", "(SUM(MinCharge) - SUM(TotalCharged)) AS Remaining", "Archived = 0 AND MinCharge > TotalCharged");
	...
	$strFilename = "/home/richdavis/Desktop/Recurring_Adjustment_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Recurring Adjustment Summary for Telco Blue");
	$wksWorksheet->writeString(0, 4, $strPageTitle, $arrFormat['PageTitle']);
	...
	//$wksWorksheet->writeString(11, 0, "Total Charge Remaining"	, $arrFormat['TextBold']);
	...
	foreach ($arrProfitData as $arrData)
	{
		...
		//$selChargeRemaining->Execute($arrData);
		//$arrChargeRemaining	= $selChargeRemaining->Fetch();
		...
		//$wksWorksheet->writeNumber(11, $intCol, $arrChargeRemaining['Remaining'], $arrFormat['Currency']);
...
function CreateReportAdjustmentsByEmployee($arrProfitData)
{
	$arrCols = Array();
	...
	$strFilename = "/home/richdavis/Desktop/Adjustments_by_Employee_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Adjustment by Employee Summary for Telco Blue");
	$wksWorksheet->writeString(0, 2, $strPageTitle, $arrFormat['PageTitle']);
	...
	$wksWorksheet->writeString(5, 6, $arrProfitData['ThisMonth']['InvoiceRun']);
...
function CreateReportInvoiceSummary($arrProfitData)
{
	$arrCols = Array();
	...
	$strFilename = "/home/richdavis/Desktop/Invoice_Summary.xls";
	@unlink($strFilename);
	$wkbWorkbook = new Spreadsheet_Excel_Writer($strFilename);
	...
	$strPageTitle = strtoupper(date("F", strtotime("-1 month", strtotime($arrProfitData['ThisMonth']['BillingDate'])))." Invoice Summary for Telco Blue");
	$wksWorksheet->writeString(0, 1, $strPageTitle, $arrFormat['PageTitle']);
