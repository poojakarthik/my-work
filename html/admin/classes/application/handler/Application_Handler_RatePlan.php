...
class Application_Handler_RatePlan extends Application_Handler
{
	...
	private static $_aAllowedBrochureMimeContentTypes = array(
		'application/pdf',
		'binary/octet-stream'
	);
	...
	public function SetBrochure($subPath)
	{
		...
		try
		{
			...
			{
				throw new Application_Handler_RatePlan_Exception_NotAllowed("You do not have sufficient privileges to upload a Plan Brochure!" . (($bolVerboseErrors) ? ' But you do have GOD mode... wtf' : ''));
			}
			...
			if (strtolower($strExtension) !== strtolower(self::PLAN_BROCHURE_FILE_EXTENSION) || !in_array($strMIME, self::$_aAllowedBrochureMimeContentTypes))
			{
				throw new Application_Handler_RatePlan_Exception_NotAllowed("'{$strFileName}' is not a valid PDF file (Extension: '{$strExtension}'; MIME: '{$strMIME}').  Ensure that you are trying to upload the correct file, and try again.");
			}
...
class Application_Handler_RatePlan_Exception_NotAllowed extends Exception {}
