...
class Application_Handler_Telemarketing extends Application_Handler
{
	...
	public function DownloadCallReconciliationReport($subPath) {
		...
		try {
			...
			$iFileImportId = (int)$_REQUEST['Telemarketing_ReconciliationDownload_File'];
			...
			$oFNNsResult	= $oQuery->Execute("
				SELECT		tfd.id AS telemarketing_fnn_dialled_id,
			...
			while ($aFNN = $oFNNsResult->fetch_assoc()) {
				...
				if ($aFNN['telemarketing_fnn_proposed_id']) {
					...
					$aRendered[self::$_aReconciliationColumns['PERMITTED_END']] = $oTelemarketingProposedFNN->call_period_end;
				}
