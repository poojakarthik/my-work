...
class Application_Handler_Telemarketing extends Application_Handler
{
	...
	public function UploadProposedDiallingList($subPath)
	{
		...
		try
		{
			...
			$strSQL		= "SELECT * FROM CarrierModule WHERE Carrier = {$objFileImport->Carrier} AND Type = ".MODULE_TYPE_TELEMARKETING." AND FileType = {$objFileImport->FileType} AND Active = 1";
