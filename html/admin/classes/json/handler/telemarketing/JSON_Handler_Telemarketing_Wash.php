...
class JSON_Handler_Telemarketing_Wash extends JSON_Handler {
	...
	public function getImportedFiles() {
		...
		try {
			...
			$resResult = $qryQuery->Execute("
				SELECT		FileImport.Id AS file_import_id,
							FileImport.FileName AS file_name,
							FileImport.ImportedOn AS file_imported_on,
							tfp.dealer_id,
							dealer.first_name AS dealer_first_name,
							dealer.last_name AS dealer_last_name,
							tfp.customer_group_id,
							CustomerGroup.external_name AS customer_group_name

				FROM		FileImport
							JOIN telemarketing_fnn_proposed tfp ON (tfp.proposed_list_file_import_id = FileImport.Id)
							JOIN CustomerGroup ON CustomerGroup.Id = tfp.customer_group_id
							JOIN dealer ON dealer.id = tfp.dealer_id

				WHERE		tfp.do_not_call_file_export_id IS NULL
							AND tfp.telemarketing_fnn_proposed_status_id = ".TELEMARKETING_FNN_PROPOSED_STATUS_IMPORTED."

				GROUP BY	FileImport.Id

				ORDER BY	file_imported_on DESC,
							file_name ASC
			");
			if ($resResult === false) {
	...
	public function getDNCRFiles() {
		...
		try {
			...
			// Get list of Imported Files
			$resResult = $qryQuery->Execute("
				SELECT		FileExport.Id AS file_export_id,
							FileExport.FileName AS file_name,
							FileExport.ExportedOn AS file_exported_on
				
				FROM		FileExport
							JOIN telemarketing_fnn_proposed tfp ON (tfp.do_not_call_file_export_id = FileExport.Id)

				WHERE		tfp.do_not_call_file_import_id IS NULL
							AND tfp.telemarketing_fnn_proposed_status_id = ".TELEMARKETING_FNN_PROPOSED_STATUS_IMPORTED."

				GROUP BY	FileExport.Id

				ORDER BY	file_exported_on DESC,
							file_name ASC
			");
			if ($resResult === false) {
				throw new Exception_Database("There was an error retrieving the required data for this page.  If this problem continues to occur, please notify YBS." . (($bolVerboseErrors) ? "\n".$qryQuery->Error() : ''));
	...
	public function getExportReadyFiles() {
		...
		try {
			...
			$resResult = $qryQuery->Execute("
				...
				WHERE		tfp.do_not_call_file_export_id IS NOT NULL
							AND tfp.do_not_call_file_import_id IS NOT NULL
							AND tfp.telemarketing_fnn_proposed_status_id = ".TELEMARKETING_FNN_PROPOSED_STATUS_IMPORTED."
	...
	public function getRecentDiallerReports() {
		$bolVerboseErrors = AuthenticatedUser()->UserHasPerm(PERMISSION_GOD);
		...
		try {
			...
			$resResult = $qryQuery->Execute("
				...
				WHERE		FileImport.ImportedOn >= SUBDATE(CURDATE(), INTERVAL 7 DAY)
