...
class JSON_Handler_Charge_Type extends JSON_Handler {
	...
	public function getAll($bActiveOnly=false) {
		...
		try {
			...
			$sMessage = $bUserIsGod ? $e->getMessage() : 'There was an error accessing the database. Please contact YBS for assistance.';
			return 	array(
				'bSuccess' => false,
				'sMessage' => $sMessage
			);
	...
	public function save($oChargeTypeDetails) {
		...
		try {
			...
			if (!isset($oChargeTypeDetails->sNature) || $oChargeTypeDetails->sNature == '' || array_search($oChargeTypeDetails->sNature, array('CR', 'DR')) === false) {
