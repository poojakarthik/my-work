...
class JSON_Handler_FollowUp_Recurring_Modify_Reason extends JSON_Handler implements JSON_Handler_Loggable
{
	public function getDataSet($bCountOnly=false, $iLimit=0, $iOffset=0, $oSort=null, $oFilter=null)
	{
		try
		{
			...
			{
				throw new JSON_Handler_FollowUp_Recurring_Modify_Reason_Exception('You do not have permission to view Recurring Follow-Up Modify Reasons.');
			}
			...
			if ($bCountOnly)
			{
				...
				return 	array(
							"Success"		=> true,
							"iRecordCount"	=> FollowUp_Recurring_Modify_Reason::searchFor(null, null, $aSort, $aFilter, true)
						);
			}
			else
			{
				...
				$aReasons	= FollowUp_Recurring_Modify_Reason::searchFor($iLimit, $iOffset, $aSort, $aFilter);
				$aResults	= array();
				$iCount		= 0;		
		...
		catch (JSON_Handler_FollowUp_Recurring_Modify_Reason_Exception $oException)
		{
			return 	array(
						"Success"	=> false,
	...
	public function getForId($iId)
	{
		try
		{
			...
			return 	array(
						"Success"		=> true,
						"oRecord"		=> FollowUp_Recurring_Modify_Reason::getForId($iId)->toStdClass()
					);
		}
		catch (JSON_Handler_FollowUp_Recurring_Modify_Reason_Exception $oException)
		{
	...
	public function deactivate($iId)
	{
		...
		try
		{
			...
			if (!AuthenticatedUser()->UserHasPerm(array(PERMISSION_OPERATOR, PERMISSION_OPERATOR_EXTERNAL)))
			{
				throw new JSON_Handler_FollowUp_Recurring_Modify_Reason_Exception('You do not have permission to deactivate Recurring Follow-Up Modify Reasons.');
			}
			
			// Change status and save
			$oReason			= FollowUp_Recurring_Modify_Reason::getForId($iId);
	...
	public function save($iId, $oDetails)
	{
		...
		try
		{
			...
			{
				$oReason	= new FollowUp_Recurring_Modify_Reason();
			}
...
class JSON_Handler_FollowUp_Recurring_Modify_Reason_Exception extends Exception
{
	// No changes
}
