...
class JSON_Handler_CDR extends JSON_Handler {
	...
	public function getCarrierList() {
		try {
			$sSql = "
				SELECT		Carrier AS Carrier, Carrier.Name AS carrier_label
				FROM		CDR
							JOIN Carrier on (CDR.Carrier = Carrier.Id)
	...
	public function getStatusList() {
		try {
			$aStatusList = array(
				CDR_BAD_OWNER => GetConstantDescription(CDR_BAD_OWNER, "CDR"),
	...
	function getPossibleOwnersForFNN($strFNN, $intServiceType) {
		try {
			return array(
				"Success" => true,
				"aData" => CDR::getPossibleOwnersForFNN($strFNN, $intServiceType)
			);
		} catch (Exception $e) {
			return array(
				"Success" => false,
	...
	function GetDelinquentCDRs($strStartDate, $strEndDate, $strFNN	,$intCarrier, $intServiceType, $iStatus=CDR_BAD_OWNER) {
		...
		try {
			return array(
				"Success" => true,
				"aRecords" => CDR::GetDelinquentCDRs($strStartDate, $strEndDate, $strFNN, $intCarrier, $intServiceType, $iStatus)
			);
		} catch (Exception $e) {
			return array(
				"Success" => false,
