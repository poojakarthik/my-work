...
class AppTemplateAccount extends ApplicationTemplate
{
	...
	function InvoicesAndPayments()
	{
		...
		DBL()->Charge->SetTable("Charge AS C LEFT OUTER JOIN Service AS S ON C.Service = S.Id LEFT JOIN ChargeType ct ON (ct.Id = (SELECT Id FROM ChargeType WHERE IF(C.charge_type_id = Id, 1, IF(C.ChargeType = ChargeType, 1, 0)) ORDER MY Id DESC LIMIT 1)");
