	...
	_renderExplorerPopup	: function(objResponse)
	{
		if (objResponse.Success)
		{
			...
			for (var i = 0; i < objResponse.objDocument.arrPath.length; i++)
			{
				...
				strBreadcrumbMenu	+= "<span class='document-explorer-address-node' "+strOnClick+"><img class='document-explorer-icon' src='"+strIcon+"' />"+objResponse.objDocument.arrPath[i].friendly_name+"</span>";
			}
			...
			if (objResponse.objDocument.arrChildren.length)
			{
				...
				for (var i = 0; i < objResponse.objDocument.arrChildren.length; i++)
				{
					...
					strDocumentListing	+=	"				<tr valign='top' onclick='"+strLink+"'>\n" +
											"					<td class='field-name' title='"+(objChild.description ? objChild.description : objChild.friendly_name)+"'>"+strIcon+"<span class='record-label'>"+strFriendlyName+"</span></td>\n" +
