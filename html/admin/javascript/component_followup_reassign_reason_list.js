...
var Component_FollowUp_Reassign_Reason_List = Class.create(
{
	initialize	: function(oContainerDiv)
	{
		...
		this.oDataSet		= 	new Dataset_Ajax(
									...
									Component_FollowUp_Reassign_Reason_List.DATA_SET_DEFINITION
								);
		this.oPagination	= 	new Pagination(
									this._updateTable.bind(this), 
									Component_FollowUp_Reassign_Reason_List.MAX_RECORDS_PER_PAGE, 
									this.oDataSet
								);
		
		// Create filter object
		this._oFilter	= new Filter(this.oDataSet, this.oPagination);
		for (var sFieldName in Component_FollowUp_Reassign_Reason_List.FILTER_FIELDS)
		{
			this._oFilter.addFilter(sFieldName, Component_FollowUp_Reassign_Reason_List.FILTER_FIELDS[sFieldName]);
		}
									...
									$T.div({class: 'section'},
										$T.div({class: 'section-header'},
											$T.div({class: 'section-header-title'},
												$T.span('Follow-Up Reassign Reasons'),
												...
												$T.thead(
													...
													$T.tr(
														this._createFieldHeader(
															'Name', 
															Component_FollowUp_Reassign_Reason_List.SORT_FIELD_NAME
														),
														this._createFieldHeader(
															'Description', 
															Component_FollowUp_Reassign_Reason_List.SORT_FIELD_DESCRIPTION
														),
														this._createFieldHeader(
															'Status', 
															Component_FollowUp_Reassign_Reason_List.SORT_FIELD_STATUS/*,
															Component_FollowUp_Reassign_Reason_List.FILTER_FIELD_STATUS*/
														),
														this._createFieldHeader('')
													)/*,
													// Filter values
													$T.tr(
														$T.th(),
														$T.th(),
														this._createFilterValueElement(Component_FollowUp_Reassign_Reason_List.FILTER_FIELD_STATUS),
														$T.th()
													)*/
												),
										...
										$T.div({class: 'section-footer'},
											$T.button({class: 'icon-button'},
												$T.img({src: Component_FollowUp_Reassign_Reason_List.ADD_IMAGE_SOURCE, alt: 'Add Reassign Reason', title: 'Add Reassign Reason'}),
												$T.span('Add Reassign Reason')
											)
										)
									)
								);
	...
	_createTableRow	: function(oReason)
	{
		if (oReason.id != null)
		{
			...
			var oEdit		= $T.img({src: Component_FollowUp_Reassign_Reason_List.EDIT_IMAGE_SOURCE, alt: 'Edit the Reassign Reason', title: 'Edit the Reassign Reason'});
			oEdit.observe('click', this._edit.bind(this, oReason.id));
			oActionsUL.appendChild(
				$T.li(oEdit)
			);
			
			var oDeactivate	= $T.img({src: Component_FollowUp_Reassign_Reason_List.INACTIVE_IMAGE_SOURCE, alt: 'Deactivate the Reassign Reason', title: 'Deactivate the Reassign Reason'});
	...
	{
		for (var sField in Component_FollowUp_Reassign_Reason_List.SORT_FIELDS)
		{
			if (this._oSort.isRegistered(sField))
			{
				...
				{
					oSortImg.src	= Component_FollowUp_Reassign_Reason_List.SORT_IMAGE_SOURCE[iDirection];
					oSortImg.show();
				}
			}
		}
	...
	{
		for (var sField in Component_FollowUp_Reassign_Reason_List.FILTER_FIELDS)
		{
	...
	{
		var oDeleteImage				= $T.img({src: Component_FollowUp_Reassign_Reason_List.REMOVE_FILTER_IMAGE_SOURCE, alt: 'Remove Filter', title: 'Remove Filter'});
	...
	_createFieldHeader	: function(sLabel, sSortField, sFilterField, bMultiLine)
	{
		...
		if (sSortField)
		{
			...
			this._oSort.registerToggleElement(oSpan, sSortField, Component_FollowUp_Reassign_Reason_List.SORT_FIELDS[sSortField]);
			this._oSort.registerToggleElement(oSortImg, sSortField, Component_FollowUp_Reassign_Reason_List.SORT_FIELDS[sSortField]);
		}
		
		// Optional filter field
		if (sFilterField)
		{
			var oIcon	= $T.img({src: Component_FollowUp_Reassign_Reason_List.FILTER_IMAGE_SOURCE, alt: 'Filter by ' + sLabel, title: 'Filter by ' + sLabel});
	...
	_edit	: function(iReasonId)
	{
		var oPopup	= 	new Popup_Record_Edit(
							'FollowUp_Reassign_Reason', 
							'Follow-Up Reassign Reason', 
							'', 
							iReasonId, 
							Component_FollowUp_Reassign_Reason_List.EDIT_FIELDS, 
	...
	_deactivate	: function(iReasonId, sReasonName, bConfirmation)
	{
		...
		else
		{
			...
			Reflex_Popup.yesNoCancel(
				"Are you sure you want to deactivate the reassign reason '" + sReasonName +"'", 
...
Component_FollowUp_Reassign_Reason_List.INACTIVE_IMAGE_SOURCE			= '../admin/img/template/decline.png';
Component_FollowUp_Reassign_Reason_List.ADD_IMAGE_SOURCE				= '../admin/img/template/new.png';

Component_FollowUp_Reassign_Reason_List.SORT_IMAGE_SOURCE						= {};
Component_FollowUp_Reassign_Reason_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_ASC]	= '../admin/img/template/order_asc.png';
Component_FollowUp_Reassign_Reason_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_DESC]	= '../admin/img/template/order_desc.png';

Component_FollowUp_Reassign_Reason_List.FILTER_FIELD_STATUS		= 'status_id';

Component_FollowUp_Reassign_Reason_List.SORT_FIELD_NAME			= 'name';
Component_FollowUp_Reassign_Reason_List.SORT_FIELD_DESCRIPTION	= 'description';
Component_FollowUp_Reassign_Reason_List.SORT_FIELD_STATUS		= 'status_id';

Component_FollowUp_Reassign_Reason_List.DATA_SET_DEFINITION		= {sObject: 'FollowUp_Reassign_Reason', sMethod: 'getDataSet'};

Component_FollowUp_Reassign_Reason_List.getAllStatusAsSelectOptions	= function(fnCallback)
{
	var aOptions	= [];
...
Component_FollowUp_Reassign_Reason_List.FILTER_FIELDS	= {};
Component_FollowUp_Reassign_Reason_List.FILTER_FIELDS.status_id	= 	{
																		...
																		oOption	: 	{
																						...
																						oDefinition	:	{
																											sLabel		: 'Owner',
																															mEditable	: true,
																															fnValidate	: null,
																															fnPopulate	: Component_FollowUp_Reassign_Reason_List.getAllStatusAsSelectOptions
																										}
																					}
																	};

// Sorting definitions
Component_FollowUp_Reassign_Reason_List.SORT_FIELDS	=	{
...
Component_FollowUp_Reassign_Reason_List.EDIT_FIELDS				= {};
Component_FollowUp_Reassign_Reason_List.EDIT_FIELDS.name			= 	{
																			sType		: 'text',
																			oDefinition	:	{
																								sLabel		: 'Name',
...
Component_FollowUp_Reassign_Reason_List.EDIT_FIELDS.description		= 	{
																			sType		: 'text',
																			oDefinition	:	{
																								sLabel		: 'Description',
...
Component_FollowUp_Reassign_Reason_List.EDIT_FIELDS.status_id		=	{
																			sType			: 'select',
