...
var Popup_Correspondence_Template = Class.create(Reflex_Popup,
{
	...
	_buildUI : function()
	{
		...
		var oSourceTypeControl =	Control_Field.factory(
										'select',
										{
											sLabel		: 'Source Type',
											mEditable	: true,
	...
	_sourceTypeChange : function(oControl)
	{
		...
		// Create new controls
		var iSourceTypeId 			= parseInt(oControl.getElementValue());
		this._hSourceTypeControls 	= {};
...
Object.extend(Popup_Correspondence_Template, 
{
	...
	_getSourceTypeOptions : function(fnCallback, oResponse)
	{
		...
		var aOptions = [];
		for (var i in oResponse.aSourceTypes)
		{
			aOptions.push(
				$T.option({value: i},
	...
	_getCarrierModuleOptions : function(fnCallback, oResponse)
	{
		...
		var aOptions = [];
		for (var i in oResponse.aModules)
		{
			var oModule = oResponse.aModules[i];
			aOptions.push(
				$T.option({value: i},
