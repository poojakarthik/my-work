FW.Package.create('FW.GUIComponent.MultipleSelect',
	{
		extends: 'FW.GUIComponent.ElementGroup',
		initialize: function($super,$values, $labels, $selectedValues, mixIsMandatory, clbValidationFunction, arrValidationEvents)
		{
			...
			for (var i = 0, l = $values.length; i < l; i++)
			{
				...
				option.selected = FW.GUIComponent.__array_contains($selectedValues, option.value);
		...
		updateDisplay: function()
		{
			...
			this.oDisplay.innerHTML = '';
				...
				for (var i = 0, l = this.aInputs[0].options.length; i < l; i++)
				{
					if (this.aInputs[0].options[i].selected)
					{
						if (matched) this.oDisplay.appendChild(document.createElement('br'));
