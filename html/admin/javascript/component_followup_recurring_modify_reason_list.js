...
var Component_FollowUp_Recurring_Modify_Reason_List = Class.create(
{
	initialize	: function(oContainerDiv)
	{
		...
		this.oDataSet		= 	new Dataset_Ajax(
									...
									Component_FollowUp_Recurring_Modify_Reason_List.DATA_SET_DEFINITION
								);
		this.oPagination	= 	new Pagination(
									this._updateTable.bind(this), 
									Component_FollowUp_Recurring_Modify_Reason_List.MAX_RECORDS_PER_PAGE, 
									this.oDataSet
								);
		
		// Create filter object
		this._oFilter	= new Filter(this.oDataSet, this.oPagination);
		for (var sFieldName in Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELDS)
		{
			this._oFilter.addFilter(sFieldName, Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELDS[sFieldName]);
		}
									...
									$T.div({class: 'section'},
										$T.div({class: 'section-header'},
											$T.div({class: 'section-header-title'},
												$T.span('Recurring Follow-Up Modification Reasons'),
												...
												$T.thead(
													...
													$T.tr(
														this._createFieldHeader(
															'Name', 
															Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_NAME
														),
														this._createFieldHeader(
															'Description', 
															Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_DESCRIPTION
														),
														this._createFieldHeader(
															'Status', 
															Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_STATUS/*,
															Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELD_STATUS*/
														),
														this._createFieldHeader('')
													)/*,
													// Filter values
													$T.tr(
														$T.th(),
														$T.th(),
														this._createFilterValueElement(Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELD_STATUS),
														$T.th()
													)*/
												),
										...
										$T.div({class: 'section-footer'},
											$T.button({class: 'icon-button'},
												$T.img({src: Component_FollowUp_Recurring_Modify_Reason_List.ADD_IMAGE_SOURCE, alt: 'Add Modification Reason', title: 'Add Modification Reason'}),
												$T.span('Add Modification Reason')
											)
										)
									)
								);
	...
	_createTableRow	: function(oReason)
	{
		if (oReason.id != null)
		{
			var sStatusClass	= '';
			switch (oReason.status_id)
			{
			...
			var	oTR	=	$T.tr(
							$T.td(oReason.name),		
							$T.td(oReason.description),
							$T.td({class: sStatusClass},
								Flex.Constant.arrConstantGroups.status[oReason.status_id].Name
							),
			...
			var oEdit		= $T.img({src: Component_FollowUp_Recurring_Modify_Reason_List.EDIT_IMAGE_SOURCE, alt: 'Edit the Modification Reason', title: 'Edit the Modification Reason'});
			oEdit.observe('click', this._edit.bind(this, oReason.id));
			oActionsUL.appendChild(
				$T.li(oEdit)
			);
			
			var oDeactivate	= $T.img({src: Component_FollowUp_Recurring_Modify_Reason_List.INACTIVE_IMAGE_SOURCE, alt: 'Deactivate the Modification Reason', title: 'Deactivate the Modification Reason'});
			oDeactivate.observe('click', this._deactivate.bind(this, oReason.id, oReason.name, false));
			oActionsUL.appendChild(
				$T.li(oDeactivate)
			);
			
			if (oReason.status_id != $CONSTANT.STATUS_ACTIVE)
			{
	...
	{
		for (var sField in Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELDS)
		{
			if (this._oSort.isRegistered(sField))
			{
				...
				{
					oSortImg.src	= Component_FollowUp_Recurring_Modify_Reason_List.SORT_IMAGE_SOURCE[iDirection];
					oSortImg.show();
				}
			}
		}
	...
	{
		for (var sField in Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELDS)
		{
	...
	{
		var oDeleteImage				= $T.img({src: Component_FollowUp_Recurring_Modify_Reason_List.REMOVE_FILTER_IMAGE_SOURCE, alt: 'Remove Filter', title: 'Remove Filter'});
	...
	_createFieldHeader	: function(sLabel, sSortField, sFilterField, bMultiLine)
	{
		...
		if (sSortField)
		{
			...
			this._oSort.registerToggleElement(oSpan, sSortField, Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELDS[sSortField]);
			this._oSort.registerToggleElement(oSortImg, sSortField, Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELDS[sSortField]);
		}
		
		// Optional filter field
		if (sFilterField)
		{
			var oIcon	= $T.img({src: Component_FollowUp_Recurring_Modify_Reason_List.FILTER_IMAGE_SOURCE, alt: 'Filter by ' + sLabel, title: 'Filter by ' + sLabel});
	...
	_edit	: function(iReasonId)
	{
		var oPopup	= 	new Popup_Record_Edit(
							...
							'Recurring Follow-Up Modification Reason', 
							'', 
							iReasonId, 
							Component_FollowUp_Recurring_Modify_Reason_List.EDIT_FIELDS, 
							this.oPagination.getCurrentPage.bind(this.oPagination)
						);
	},
	
	_deactivate	: function(iReasonId, sReasonName, bConfirmation)
	{
		if (bConfirmation)
		{
			// Got confirmation, deactive the Reason
			...
			fnDeactivate(iReasonId);
		}
		else
		{
			// Show confirmation popup
			Reflex_Popup.yesNoCancel(
				"Are you sure you want to deactivate the modification reason '" + sReasonName +"'", 
				{fnOnYes: this._deactivate.bind(this, iReasonId, sReasonName, true)}
			);
		}
	}
});

Component_FollowUp_Recurring_Modify_Reason_List.MAX_RECORDS_PER_PAGE		= 10;
Component_FollowUp_Recurring_Modify_Reason_List.FILTER_IMAGE_SOURCE			= '../admin/img/template/table_row_insert.png';
Component_FollowUp_Recurring_Modify_Reason_List.REMOVE_FILTER_IMAGE_SOURCE	= '../admin/img/template/delete.png';
Component_FollowUp_Recurring_Modify_Reason_List.EDIT_IMAGE_SOURCE			= '../admin/img/template/pencil.png';
Component_FollowUp_Recurring_Modify_Reason_List.INACTIVE_IMAGE_SOURCE		= '../admin/img/template/decline.png';
Component_FollowUp_Recurring_Modify_Reason_List.ADD_IMAGE_SOURCE			= '../admin/img/template/new.png';

Component_FollowUp_Recurring_Modify_Reason_List.SORT_IMAGE_SOURCE						= {};
Component_FollowUp_Recurring_Modify_Reason_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_ASC]	= '../admin/img/template/order_asc.png';
Component_FollowUp_Recurring_Modify_Reason_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_DESC]	= '../admin/img/template/order_desc.png';

Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELD_STATUS		= 'status_id';

Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_NAME			= 'name';
Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_DESCRIPTION	= 'description';
Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELD_STATUS		= 'status_id';

Component_FollowUp_Recurring_Modify_Reason_List.DATA_SET_DEFINITION		= {sObject: 'FollowUp_Recurring_Modify_Reason', sMethod: 'getDataSet'};

Component_FollowUp_Recurring_Modify_Reason_List.getAllStatusAsSelectOptions	= function(fnCallback)
{
	var aOptions	= [];
...
Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELDS	= {};
Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELDS[Component_FollowUp_Recurring_Modify_Reason_List.FILTER_FIELD_STATUS]	= 	{
																																			...
																																			oOption	: 	{
																																							...
																																							oDefinition	:	{
																																												...
																																												mEditable	: true,
																																												fnValidate	: null,
																																												fnPopulate	: Component_FollowUp_Recurring_Modify_Reason_List.getAllStatusAsSelectOptions
																																											}
																																						}
																																		};

// Sorting definitions
Component_FollowUp_Recurring_Modify_Reason_List.SORT_FIELDS	=	{
...
Component_FollowUp_Recurring_Modify_Reason_List.EDIT_FIELDS					= {};
Component_FollowUp_Recurring_Modify_Reason_List.EDIT_FIELDS.name			= 	{
																					sType		: 'text',
																					oDefinition	:	{
																										sLabel		: 'Name',
...
Component_FollowUp_Recurring_Modify_Reason_List.EDIT_FIELDS.description		= 	{
																					sType		: 'text',
																					oDefinition	:	{
																										sLabel		: 'Description',
...
Component_FollowUp_Recurring_Modify_Reason_List.EDIT_FIELDS.status_id		=	{
																					sType			: 'select',
