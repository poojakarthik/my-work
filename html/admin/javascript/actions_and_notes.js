...
Object.extend(ActionsAndNotes, {
	...
	load : function(funcOnLoaded) {
		...
		this.arrFuncsOnLoaded = [funcOnLoaded];
		...
		Flex.Constant.loadConstantGroup(['action_association_type', 'action_type_detail_requirement', 'active_status', 'followup_type'], this._onLoaded.bind(this, "Loading Constant Groups"));
	},
	...
	fireEvent : function(strEventType) {
		var eventType = String(strEventType);
...
Object.extend(ActionsAndNotes.Creator.prototype, {
	...
	submit : function(bolConfirmed) {
		...
		if (this.elmSelectedTypeOption.isNoteType) {
			...
			jsonFunc = jQuery.json.jsonFunction(this.submitNoteResponse.bind(this, note), null, "ActionsAndNotes", "createNote");
			...
			jsonFunc = jQuery.json.jsonFunction(this.submitActionResponse.bind(this, action), null, "ActionsAndNotes", "createAction");
	...
	detailsRequired : function() {
		return (
			(this.elmSelectedTypeOption.isNoteType) ||
	...
	getDetailsString : function() {
		var strDetails = String(this.elmDetailsTextarea.value);
...
Object.extend(ActionsAndNotes.List.prototype, {
	...
	*/
	...
	initialize : function(intAATContextId, intAATContextReferenceId, bolIncludeAllRelatableAATTypes, intMaxRecordsPerPage) {
		...
		this.elmTypeConstraintCombobox.style.margin = '0';
		...
		this.elmLoggedByConstraintCombobox.style.margin = '0';

		// Textual Search Input
		// HACKHACKHACK: Inline styling, because that's how all of the styling is done here
		this.oSearchElement = $T.input({type:'search', placeholder:'Search using Reference Numbers or keywords...'});
		this.oSearchElement.setStyle({
			display : 'block',
			width : '100%'
		});
		...
		this.elmFilterControlsContainerDiv.appendChild(this.oSearchElement);
	...
	search : function(bolUseLast, intPageOffset) {
		...
		if (bolUseLast && this.lastSearch != null) {
			...
			sSearchConstraint = this.lastSearch.searchString;
			...
			sSearchConstraint = this.oSearchElement.value;
			intPageOffset = 0;
		}

		if (!isNaN(typeConstraint)) {
			// The typeConstraint is an action_type_id
			typeConstraint = parseInt(typeConstraint, 10);
		}

		this.lockButtons([this.elmFirstButton, this.elmPreviousButton, this.elmNextButton, this.elmLastButton]);
		this.jsonFuncSearch(this.intAATContextId, this.intAATContextReferenceId, this.bolIncludeAllRelatableAATTypes, typeConstraint, loggedByConstraint, sSearchConstraint, this.intMaxRecordsPerPage, intPageOffset);
	},
	...
	setLastSearchDetails : function() {
		...
		if (this.lastSearch != null && this.lastSearch.pageRecordCount > 0) {
			...
			this.lockButtons([this.elmSearchButton], false);
		}
	...
	renderItem : function(objItem, bInsertFollowUpPlaceholder) {
		...
		bInsertFollowUpPlaceholder	= ((bInsertFollowUpPlaceholder === false) ? false : true);
