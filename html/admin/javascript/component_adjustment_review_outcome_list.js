...
var Component_Adjustment_Review_Outcome_List = Class.create(
{
	initialize	: function(iPageSize, oLoadingPopup)
	{
		...
		this._oElement				= $T.div({class: 'component-adjustment-review-outcome-list'});
		this._oLoadingPopup			= oLoadingPopup;
		this._oOverlay 				= new Reflex_Loading_Overlay();
		
		// Load constants then create UI
		Flex.Constant.loadConstantGroup(Component_Adjustment_Review_Outcome_List.REQUIRED_CONSTANT_GROUPS, this._buildUI.bind(this));
	},
	...
	_buildUI : function()
	{
		...
		this.oDataSet		= 	new Dataset_Ajax(Dataset_Ajax.CACHE_MODE_NO_CACHING, Component_Adjustment_Review_Outcome_List.DATA_SET_DEFINITION);
		this.oPagination 	= 	new Pagination(
									this._updateTable.bind(this), 
									(this._iPageSize ? this._iPageSize : Component_Adjustment_Review_Outcome_List.MAX_RECORDS_PER_PAGE), 
									this.oDataSet
								);
		...
		for (var sFieldName in Component_Adjustment_Review_Outcome_List.FILTER_FIELDS)
		{
			if (Component_Adjustment_Review_Outcome_List.FILTER_FIELDS[sFieldName].iType)
			{
				this._oFilter.addFilter(sFieldName, Component_Adjustment_Review_Outcome_List.FILTER_FIELDS[sFieldName]);
			}
		}
		...
		this._oDataSetSelection = new Dataset_Ajax(Dataset_Ajax.CACHE_MODE_NO_CACHING, Component_Adjustment_Review_Outcome_List.DATA_SET_DEFINITION);
		...
		oSection.setTitleContent(
			$T.span(
				$T.span('Adjustment Review Outcomes'),
		...
		oSection.addToHeaderOptions(
			$T.button({class: 'icon-button'},
				...
				$T.span('Add Declined Outcome')	
			).observe('click', this._addOutcome.bind(this))
		);
		
		// Main content -- table
		oSection.setContent(
			$T.table({class: 'component-adjustment-review-outcome-list-table reflex highlight-rows'},
				$T.thead(
					// Column headings
					$T.tr({class: 'component-adjustment-review-outcome-list-headerrow'},
						$T.th('#'),
						$T.th('Name'),
						$T.th('Description'),
						$T.th('Review Outcome Type'),
						$T.th('System'),
						$T.th('Status'),
						$T.th('') // Actions
					),
					...
					$T.tr(
						...
						$T.th(),
						$T.th(),
						this._createFilterValueElement('adjustment_review_outcome_type_id', 'Review Outcome Type'),
						$T.th(),
		...
		// ... account id
		var aHeaderRowTHs = this._oElement.select('.component-adjustment-review-outcome-list-headerrow > th');
		...
		this._registerSortHeader(aHeaderRowTHs[3], 'adjustment_review_outcome_type_name');
		this._registerSortHeader(aHeaderRowTHs[4], 'is_system');
		this._registerSortHeader(aHeaderRowTHs[5], 'status_name');
		
		// Footer -- loading msg & pagination
		oSection.setFooterContent(
			$T.ul({class: 'reset horizontal component-adjustment-review-outcome-list-options'},
				$T.li(
					$T.button({class: 'component-adjustment-review-outcome-list-paginationbutton'},
		...
		var aBottomPageButtons 	= this._oElement.select('.component-adjustment-review-outcome-list-paginationbutton');
		...
		if (oData.id)
		{
			...
			switch (oData.status_id)
			{
				case $CONSTANT.STATUS_ACTIVE:
					...
					fnStatusOnClick	= this._setOutcomeStatus.bind(this, oData.id, $CONSTANT.STATUS_INACTIVE, null);
					break;
				case $CONSTANT.STATUS_INACTIVE:
					sStatusIconText = 'Enable';
					sStatusIconSrc 	= '../admin/img/template/approve.png';
					fnStatusOnClick	= this._setOutcomeStatus.bind(this, oData.id, $CONSTANT.STATUS_ACTIVE, null);
					break;
			}
			...
			var	oTR	=	$T.tr(
							...
							$T.td(oData.adjustment_review_outcome_type_name),
							$T.td(oData.is_system ? 'Yes' : 'No'),
	...
	{
		for (var sField in Component_Adjustment_Review_Outcome_List.SORT_FIELDS)
		{
			if (this._oSort.isRegistered(sField))
			{
				...
				oSortImg.src	= Component_Adjustment_Review_Outcome_List.SORT_IMAGE_SOURCE[iDirection];
				oSortImg.show();
			}
		}
	},
	...
	{
		for (var sField in Component_Adjustment_Review_Outcome_List.FILTER_FIELDS)
		{
			if (Component_Adjustment_Review_Outcome_List.FILTER_FIELDS[sField].iType)
			{
	...
	_updateFilterDisplayValue	: function(sField)
	{
		if (this._oFilter.isRegistered(sField))
		{
			...
			var oSpan	= this._oElement.select('th.component-adjustment-review-outcome-list-filter-heading > span.filter-' + sField).first();
			if (oSpan)
			{
				var oDeleteImage	= oSpan.up().select('img.component-adjustment-review-outcome-list-filter-delete').first();
	...
	{
		var oDeleteImage				= $T.img({class: 'component-adjustment-review-outcome-list-filter-delete', src: Component_Adjustment_Review_Outcome_List.REMOVE_FILTER_IMAGE_SOURCE, alt: 'Remove Filter', title: 'Remove Filter'});
		oDeleteImage.style.visibility	= 'hidden';
		oDeleteImage.observe('click', this._clearFilterValue.bind(this, sField));

		var oFiterImage	= $T.img({class: 'header-filter', src: Component_Adjustment_Review_Outcome_List.FILTER_IMAGE_SOURCE, alt: 'Filter by ' + sLabel, title: 'Filter by ' + sLabel});
		this._oFilter.registerFilterIcon(sField, oFiterImage, sLabel, this._oElement, 0, 10);

		return	$T.th({class: 'component-adjustment-review-outcome-list-filter-heading'},
	...
	_registerSortHeader : function(oElement, sSortField)
	{
		...
		oSpan.addClassName('component-adjustment-review-outcome-list-header-sort');
		
		this._oSort.registerToggleElement(oSpan, sSortField, Component_Adjustment_Review_Outcome_List.SORT_FIELDS[sSortField]);
		this._oSort.registerToggleElement(oSortImg, sSortField, Component_Adjustment_Review_Outcome_List.SORT_FIELDS[sSortField]);
	},
	
	_formatFilterValueForDisplay	: function(sField, mValue)
	{
		var oDefinition	= Component_Adjustment_Review_Outcome_List.FILTER_FIELDS[sField];
		var aControls	= this._oFilter.getControlsForField(sField);
		var sValue		= '';
		switch (sField)
		{
			case 'status_id':
			case 'adjustment_review_outcome_type_id':
	...
	_setOutcomeStatus : function(iOutcomeId, iStatusId, oResponse)
	{
		if (!oResponse)
		{
			// Make request
			...
			var fnResp 	= this._setOutcomeStatus.bind(this, iOutcomeId, iStatusId);
			var fnReq	= jQuery.json.jsonFunction(fnResp, fnResp, 'Adjustment_Review_Outcome', 'setStatus');
			fnReq(iOutcomeId, iStatusId);
			return;
		}
	...
	_addOutcome : function()
	{
		new Popup_Adjustment_Review_Outcome(this.oPagination.getCurrentPage.bind(this.oPagination));
	}
});

// Static

Object.extend(Component_Adjustment_Review_Outcome_List,
{
	DATA_SET_DEFINITION			: {sObject: 'Adjustment_Review_Outcome', sMethod: 'getDataset'},
...
Component_Adjustment_Review_Outcome_List.SORT_IMAGE_SOURCE						= {};
Component_Adjustment_Review_Outcome_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_OFF]	= '../admin/img/template/order_neither.png';
Component_Adjustment_Review_Outcome_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_ASC]	= '../admin/img/template/order_asc.png';
Component_Adjustment_Review_Outcome_List.SORT_IMAGE_SOURCE[Sort.DIRECTION_DESC]	= '../admin/img/template/order_desc.png';

//Filter Control field definitions (starts at 
Component_Adjustment_Review_Outcome_List.FILTER_FIELDS	=
{
	status_id	:
	{
	...
	adjustment_review_outcome_type_id	:
	{
		iType	: Filter.FILTER_TYPE_VALUE,
		oOption	:
		{
			...
			oDefinition	:
			{
				...
				mEditable	: true,
				mMandatory	: false,
				fnPopulate	: Flex.Constant.getConstantGroupOptions.curry('adjustment_review_outcome_type')
			}
		}
	}
};
