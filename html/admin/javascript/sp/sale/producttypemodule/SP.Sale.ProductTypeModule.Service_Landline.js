FW.Package.create('SP.Sale.ProductTypeModule.Service_Landline', {
	requires: ['SP.Sale.ProductTypeModule.Service_Landline.LineType', 'SP.Sale.ProductTypeModule.Service_Landline.LineType.Residential', 'SP.Sale.ProductTypeModule.Service_Landline.LineType.Business','FW.GUIComponent.CheckboxGroup','FW.GUIComponent.TextInputGroup','FW.GUIComponent.DropDown'],
	...
	buildGUI: function()
	{
		...
		this.detailsContainer.innerHTML = ''
		 ...
		 + '<table style="border-colapse: collapse; margin: 0; padding: 0;">'
			 + '<tr>'
				 ...
				 + '<td style="width: 50%; padding: 0;">'
					 + '<h3 style="position: relative; width: 100%;" class="data-entry">Service Address<input type="button" value="Copy from another service" style="width: 180px; position: absolute; right: 0px; bottom: -1px;" onclick="SP.Sale.ProductTypeModule.Service_Landline.showCopyWindow(\'' + this.uniqueId + '\');" /></h3>'
		...
		this.addElementGroup('fnn', new FW.GUIComponent.TextInputGroup(this.getFNN(), true, window._validate.fnnLandLine.bind(this)),'Landline Phone Number');
		...
		this.addElementGroup('is_indial_100', new FW.GUIComponent.CheckboxGroup(this.getIsIndial100(), false, fncIndialIsValid.bind(this)),'Is Indial 100');
		this.addElementGroup('has_extension_level_billing', new FW.GUIComponent.CheckboxGroup(this.getHasExtensionLevelBilling()),'Has Extension Level Billing');
		this.elementGroups.is_indial_100.isValid();

		
		
		this.addElementGroup('landline_type_id', new FW.GUIComponent.DropDown(
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineType.id,
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineType.description,
		...
		var lengths = SP.Sale.ProductTypeModule.Service_Landline.staticData.lengths;

		this.addElementGroup('bill_name', new FW.GUIComponent.TextInputGroup(this.getBillName(), true, fncGetStringValidationFunc(lengths.landline.billName)),'Name');
		this.addElementGroup('bill_address_line_1', new FW.GUIComponent.TextInputGroup(this.getBillAddressLine1(), true, fncGetStringValidationFunc(lengths.landline.billAddressLine1)),'Address (Line 1)');
		this.addElementGroup('bill_address_line_2', new FW.GUIComponent.TextInputGroup(this.getBillAddressLine2(), false, fncGetStringValidationFunc(lengths.landline.billAddressLine2)),'Address (Line 2)');
		this.addElementGroup('bill_locality', new FW.GUIComponent.TextInputGroup(this.getBillLocality(), true, fncGetStringValidationFunc(lengths.landline.billLocality)),'Suburb');
		this.addElementGroup('bill_postcode', new FW.GUIComponent.TextInputGroup(this.getBillPostcode(), true, window._validate.postcode.bind(this)),'Postcode');
		...
		this.addElementGroup('landline_service_address_type_id', new FW.GUIComponent.DropDown(
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceAddressType.id,
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceAddressType.description,
			this.getLandlineServiceAddressTypeId()),'Address Type');
		...
		this.addElementGroup('service_address_type_number', new FW.GUIComponent.TextInputGroup(this.getServiceAddressTypeNumber(), fncMandatoryAddressTypeNumber.bind(this), window._validate.integerPositive.bind(this)),'Address Type Number');
		...
		this.addElementGroup('service_address_type_suffix', new FW.GUIComponent.TextInputGroup(this.getServiceAddressTypeSuffix(), false, fncGetStringValidationFunc(lengths.landline.serviceAddressTypeSuffix)),'Address Type Suffix');
				
		this.getWorkingTable().rows[this.getWorkingTable().rows.length-1].className += " data-advanced";
		this.addElementGroup('service_street_number_start', new FW.GUIComponent.TextInputGroup(this.getServiceStreetNumberStart(), null, window._validate.integerPositive.bind(this)),'Street Number Start');
		var fnIsMandatory = function()
							{
								return (	(   this.isAdvancedAddress.checkbox.checked
											...
											|| (!this.isAdvancedAddress.checkbox.checked));//fncStreetNumberMandatory
							}
		...
		this.addElementGroup('service_street_number_end', new FW.GUIComponent.TextInputGroup(this.getServiceStreetNumberEnd(), false, window._validate.integerPositive.bind(this)),'Street Number End');
		this.getWorkingTable().rows[this.getWorkingTable().rows.length-1].className += " data-advanced";

		this.addElementGroup('service_street_number_suffix', new FW.GUIComponent.TextInputGroup(this.getServiceStreetNumberSuffix(), false, fncGetStringValidationFunc(lengths.landline.serviceStreetNumberSuffix)),'Street Number Suffix');
		this.getWorkingTable().rows[this.getWorkingTable().rows.length-1].className += " data-advanced";

		this.addElementGroup('service_street_name', new FW.GUIComponent.TextInputGroup(this.getServiceStreetName(), null, fncGetStringValidationFunc(lengths.landline.serviceStreetName)),'Street Name');
		...
		this.addElementGroup('landline_service_street_type_id', new FW.GUIComponent.DropDown(
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceStreetType.id,
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceStreetType.description,
			this.getLandlineServiceStreetTypeId(),
			fncStreetTypeMandatory.bind(this)),'Street Type');
		
		this.addElementGroup('landline_service_street_type_suffix_id', new FW.GUIComponent.DropDown(
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceStreetTypeSuffix.id,
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceStreetTypeSuffix.description,
			this.getLandlineServiceStreetTypeSuffixId()),'Street Type Suffix');
		
		fncPropertyNameMandatory =  function()
									{
										return !(this.elementGroups.service_street_name.getValue());
									};
		this.addElementGroup('service_property_name', new FW.GUIComponent.TextInputGroup(this.getServicePropertyName(), fncPropertyNameMandatory.bind(this), fncGetStringValidationFunc(lengths.landline.servicePropertyName)),'Property Name');
		...
		this.addElementGroup('service_locality', new FW.GUIComponent.TextInputGroup(this.getServiceLocality(), true, fncGetStringValidationFunc(lengths.landline.serviceLocality)),'Suburb');
		this.addElementGroup('service_postcode', new FW.GUIComponent.TextInputGroup(this.getServicePostcode(), true, window._validate.postcode.bind(this)),'Postcode');
		this.addElementGroup('landline_service_state_id', new FW.GUIComponent.DropDown(
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceState.id,
			SP.Sale.ProductTypeModule.Service_Landline.staticData.landlineServiceState.description,
	...
	setLandlineTypeId: function(value)
	{
		...
		if (   (value != this.object.landline_type_id)
			|| (value ==SP.Sale.ProductTypeModule.Service_Landline.LineType.LINE_TYPE_RESIDENTIAL && this.landlineAddressDetails instanceof SP.Sale.ProductTypeModule.Service_Landline.LineType.Business)
			|| (value == SP.Sale.ProductTypeModule.Service_Landline.LineType.LINE_TYPE_BUSINESS && 	this.landlineAddressDetails instanceof SP.Sale.ProductTypeModule.Service_Landline.LineType.Residential))
		{
		...
		{
			this.landlineAddressDetails = new ((value == SP.Sale.ProductTypeModule.Service_Landline.LineType.LINE_TYPE_RESIDENTIAL)
					? SP.Sale.ProductTypeModule.Service_Landline.LineType.Residential
					: SP.Sale.ProductTypeModule.Service_Landline.LineType.Business)(this.object.landline_type_details);
	...
	{
		intValue	= parseInt(this.elementGroups.landline_service_address_type_id.getValue());
	...
	{
		intValue	= parseInt(this.elementGroups.service_address_type_number.getValue());
