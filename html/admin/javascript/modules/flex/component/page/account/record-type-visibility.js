...
var self = new Class({
				...
				H.table({class: 'reflex highlight-rows'},
					H.caption(
						H.div({id: 'caption_bar', class: 'caption_bar'},
							H.div({id: "caption_title", class: "caption_title"}, 'Account Record Type Visibility'),
		...
		for(var iIndex in oRecordTypes){
			if(oRecordTypes.hasOwnProperty(iIndex)) {
				var oCheckbox = new Checkbox({
					...
					sName: oRecordTypes[iIndex].account_record_type_visibility_id
				});
	...
	_save : function(oData, fnCallback) {
		new Ajax.Request('/admin/reflex_json.php/Account_Record_Type_Visibility/updateIsVisibleForId', {
			method		: 'post',
