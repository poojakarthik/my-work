...
var	self = new Class({
	...
	construct : function() {
		...
		this.NODE.addClassName('flex-page-account-record-type-visibility-new');
	},
	...
	_buildUI : function() {
		this._oForm = new Form({onsubmit: this._save.bind(this, null)},
			...
			H.fieldset(
				...
				H.div(
					this._oRecordType = new Select({
						fnPopulate : this._getUnusedRecordTypesForAccountId.bind(this)
					})
				),
				H.div(
					H.span('Visibility')
			...
			H.div({'class': 'flex-page-account-record-type-visibility-new-buttons'},
	...
	_save : function(fnCallback, oXHREvent) {
		...
		new Ajax.Request('/admin/reflex_json.php/Account_Record_Type_Visibility/saveRecordType', {
			method		: 'post',
	...
	_getUnusedRecordTypesForAccountId : function(fnCallback, oXHREvent) {
		var oData = {
			iAccountId : this.get('iAccountId')
		};
		new Ajax.Request('/admin/reflex_json.php/Account_Record_Type_Visibility/getUnusedRecordTypesForAccountId', {
			method		: 'post',
			contentType	: 'application/x-www-form-urlencoded',
			postBody	: "json="+encodeURIComponent(JSON.stringify([oData])),
			onSuccess: function (oResponse){
...
Object.extend(self, {
	...
	createAsPopup : function() {
		...
		oPopup			= new Popup({
				...
				sTitle			: 'New Account Record Type Visibility',
				bCloseButton	: true
			},
			oComponent.getNode()
		);
