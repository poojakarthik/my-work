...
var     self = new Class({
		...
		_syncUI : function() {
			if (!this._bInitialised || !this._onReady) {
				...
				if(this.get('existingConstraints')) {

					var currentList = this.get('existingConstraints');
					if(currentList.length>0) {
						// Populate existing constraints
						for (var i = 0; i < currentList.length; i++) {
							console.log(currentList[i]);
							this._constraintList.appendChild(
									H.tr({class: 'First'},
										H.td({align: 'Left'}, currentList[i].name),
										H.td({align: 'Left'}, currentList[i].type),
										H.td({align: 'Left'}, '')
									)
								)
						};
