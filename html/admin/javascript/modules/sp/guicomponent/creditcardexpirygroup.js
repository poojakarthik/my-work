...
var self = new Class({
	...
	construct : function ($values, mixIsMandatory, clbValidationFunction, arrValidationEvents) {
	...
	getValue : function () {
		this.updateDisplay();
		if (parseInt(this.aInputs[0].options[this.aInputs[0].selectedIndex].value, 10) && parseInt(this.aInputs[1].options[this.aInputs[1].selectedIndex].value, 10)) {
			return [this.aInputs[0].options[this.aInputs[0].selectedIndex].value, this.aInputs[1].options[this.aInputs[1].selectedIndex].value];
		}
		return null;
	},
