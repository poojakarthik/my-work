...
var self = new Class({
	...
	construct : function (mIsMandatory, fnIsValidCustom, aValidationEvents) {
		mIsMandatory = ((mIsMandatory == null) ? false : mIsMandatory);
		this.mIsMandatory =  mIsMandatory;
		this.fnIsValidCustom = fnIsValidCustom;
		if (aValidationEvents == null) {
			aValidationEvents	= ['change', 'keyup', 'click'];
		}
	...
	isValid : function () {
		...
		var strValue = (mixValue === null) ? '' : String(mixValue).strip();
		...
		} else if (this.mIsMandatory == null) {
		...
		var bolValid	= (this.fnIsValidCustom == null) ? true : this.fnIsValidCustom(strValue);
