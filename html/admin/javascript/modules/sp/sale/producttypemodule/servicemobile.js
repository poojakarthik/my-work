...
var self = new Class({
	...
	updateSummary : function (suggestion) {
		var noFNN = (this.object.fnn == null || this.object.fnn == '');
	...
	buildGUI : function () {
		var id = 'service-mobile-table-' + (self.unique++);
		this.detailsContainer.innerHTML = '<table id="'+id+'" class="data-table"></table>';

		this.setWorkingTable($ID(id));

		this.addElementGroup('service_mobile_origin_id', new DropDown(
			self.staticData.serviceMobileOrigin.id,
			self.staticData.serviceMobileOrigin.description,
			this.getServiceMobileOriginId(),
			true
		), 'Origin');
		
		this.addElementGroup('fnn', new TextInputGroup(this.getFNN(), this.isExistingService.bind(this), Validation.fnnMobile.bind(this)),'Mobile Phone Number');
		this.addElementGroup('sim_puk', new TextInputGroup(this.getSimPUK(), false, Validation.getStringLengthValidationFunc(self.staticData.lengths.simPuk)),'SIM PUK');
		this.addElementGroup('sim_state_id', new DropDown(Sale.states.ids, Sale.states.labels, this.getSimStateId()),'SIM State');
		this.addElementGroup('dob', new DateGroup(this.getDOB(), this.isExistingPrePaid.bind(this), Validation.date.bind(this)),'Date of Birth');
		
		this.addElementGroup('current_provider', new TextInputGroup(this.getCurrentProvider() ),'Current Provider');
		...
		this.addElementGroup('current_account_number', new TextInputGroup(this.getExistingAccountNumber(), fncMandatoryCurrentAccount.bind(this)),'Existing Account Number');
		
		this.addElementGroup('comments', new TextInputGroup(this.getComments()),'Comments');
