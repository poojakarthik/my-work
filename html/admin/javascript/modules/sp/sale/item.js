...
var self = new Class({
	...
	isValid : function () {
		return this.getProductModule().isValid();
	},
	...
	getProductModule : function () {
		//debugger;
		if (this.productModule == null) {
			this.productModule = ProductTypeModule.getModuleInstance(this.object.product_type_module, this.object.product_detail);
			if (this.productModule != null) {
				this.object.product_detail = this.productModule.object;
			} else {
				this.productModule = null;
			}
		}
	...
	statics : {
		...
		register : function (instance) {
			instance.instanceId = 'sale-item-' + (self.unique++);
			self.instances[instance.instanceId] = instance;
		},
		
		deregister : function (instance) {
			delete self.instances[instance.instanceId];
		},
		...
		collapseAll : function () {
			for (var instanceId in self.instances) {
				if (typeof self.instances[instanceId] == 'function') {
					continue;
				}
				self.instances[instanceId].collapse();
			}
		},
		...
		expandAll : function () {
			for (var instanceId in self.instances) {
				...
				self.instances[instanceId].expand();
			}
		}
	}
});
