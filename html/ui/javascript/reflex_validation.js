...
var Reflex_Validation = {
	...
	abn : function (mixValue) {
		var strValue = mixValue.toString().replace(/[^\d]/g, '');
		...
		var strABNStep1 = "" + (parseInt(strValue.charAt(0), 10) - 1) + strValue.substr(1);
		...
		for (i=0; i < 11; i++) {
			intABNStep3 += parseInt(strABNStep1.charAt(i), 10) * arrWeight[i];
		}
	...
	acn : function (mixValue) {
		...
		for (i=0; i < 8; ++i) {
			NumberSum += parseInt(intACN.charAt(i) * arrWeights[i], 10);
		}
	...
	tioReferenceNumber : function (mValue) {
		return (
			// Old style
			mValue.toString().match(/^\d{2}\/\d{6,7}$/) ||
			// New style
			mValue.toString().match(/^\d{4}\/\d{2}\/\d{2,}$/)
		);
	},
	...
	Exception : {
		...
		tioReferenceNumber : function (mValue) {
			if (Reflex_Validation.tioReferenceNumber(mValue)) {
				...
				throw ('Invalid TIO Reference Number, must be {2 digits}/{6/7 digits} (old format) or {4 digits}/{2 digits}/{2+ digits} (new format)');
			}
		},
