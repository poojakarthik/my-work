...
function DateChooser(input, div, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear)
{
	...
	this._prefix = "_DateChooser_" + this.instanceCount + "_";
	this._input._prefix = this._prefix;
	DateChooser.instances[this._prefix] = this;
	this.instanceCount++;
}
...
DateChooser.showChooser = function(input, div, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear)
{
	if (input !== undefined && div !== undefined)
	{
		var dateChooser = DateChooser.instances[input._prefix];
		if (dateChooser === undefined)
		{
			dateChooser = new DateChooser(input, div, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear);
		}
		dateChooser._setDate(Date.parseDate(input.value, format));
...
DateChooser.prototype = {
	...
	show: function() {
		...
		else
		{
			...
			{
				this._yearSelect.selectedIndex = this._date.getYear();
			}
			...
			{
				this._hourSelect.selectedIndex = this._date.getHours() % 12;
			}
	...
	createDateChooserHtml: function()
	{
		...
		select.id = this._prefix + "month";
		select.addEventListener("change", new Function("DateChooser.dateChange('" + this._prefix + "');"), false);
		...
		img.addEventListener("click", new Function("DateChooser.setDate('" + this._prefix + "', null);"), false);
	...
	createCalendarHtml: function()
	{
		...
		while (i < daysInMonth) {
			...
			var js = "DateChooser.setDate('" + this._prefix + "', '" + thisDay.dateFormat(this._format) + "');";
			var cell = row.insertCell(-1);
