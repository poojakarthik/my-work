...
function showChooser(obj, inputId, divId, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear)
{
    if (document.getElementById)
	{
        ...
        if (input !== undefined && div !== undefined)
		{
			...
			{
                input.DateChooser = new DateChooser(input, div, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear);
            }
...
function DateChooser(input, div, start, end, format, isTimeChooser, isDateChooser, useCalendar, defaultYear) {
    this._input = input;
    this._div = div;
    this._start = start;
    this._end = end;
    this._default = defaultYear;
    this._format = format;
    this._date = new Date();
...
// Returns true if the chooser is to allow choosing the time as well as the date
DateChooser.prototype.isDateChooser = function() {
    return this._isDateChooser;
}

// Returns true if the chooser is to allow choosing the time as well as the date
DateChooser.prototype.useCalendar = function() {
...
DateChooser.prototype.show = function() {
	...
	this._div.style.left = (this._input.offsetLeft + this._input.clientWidth + 22) + 'px';
	this._div.style.top = (this._input.offsetTop + this._input.clientHeight - 18) + 'px';
...
DateChooser.prototype.initializeDate = function() {
    ...
    else if (this._default != undefined && this._default != "") {
        this._date = new Date();
    }
    else {
        this._date = new Date();
...
DateChooser.prototype.getDefaultDate = function(date) {
	var defaultDate = new Date();
	if (this._default != undefined)
	{
		defaultDate.setFullYear(this._default);
	}
	return defaultDate;
}
...
DateChooser.prototype.createChooserHtml = function() {
    var formHtml = "<input type=\"hidden\" name=\""
	    ...
	    formHtml += "<div class=\"bar\"><select name=\"" + this._prefix 
...
DateChooser.prototype.createTimeChooserHtml = function() {
    // Add hours
    var result = "\r\n  <div class=\"bar\"><select name=\"" + this._prefix + "hour\">";
    for (var i = 0; i < 12; ++i) {
        result += "\r\n    <option value=\"" + i + "\""
...
DateChooser.prototype.createCalendarHtml = function() {
    ...
    while (i < daysInMonth) {
        ...
        result += "\r\n    <td class=\"date-time-active"
            // If the date is the currently chosen date, highlight it
            + (i == selectedDate ? " date-time-active-today" : "")
            + "\" onclick=" + js + ">" + i + "</td>";
    }
...
// This does not work properly yet.  Do not use.
function CreateDatePickerPopup(obj, inputId, start, end, format, isTimeChooser, isDateChooser, useCalendar)
{
	var strContent = 	"<div align='center' style='margin: 5px; width=190px; height:140px'>" +
						"<div id='to_date_placeholder' class='date-time select-free' style='display: none; visibility: hidden;'></div></div>";

	// open the popup
	Vixen.Popup.Create("DateTimePicker", strContent, "small", "centre", "modal", "DateTimePicker");

	// show the chooser
	showChooser(obj, inputId, "to_date_placeholder", start, end, format, isTimeChooser, isDateChooser, useCalendar);
