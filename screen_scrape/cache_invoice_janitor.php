<?php
	...
	$rptReport = new Report (
		"+	ETECH INVOICE JANITOR : " . date ("Y-m-d h:i:s A"),
	...
	$selScrape = new StatementSelect ('ScrapeInvoice', 'CustomerId, Attempt, TimeTaken', 'DataOriginal = 1 OR DataOriginal = ""', 'RAND()', '1');
	...
	$updScrape = new StatementUpdate ('ScrapeInvoice', 'CustomerId = <CustomerId>', $arrScrapeSave);
	...
	while (true)
	{
		...
		$strResponse = $cnnConnection->Transmit (
			"GET",
			"https://sp.teleconsole.com.au/sp/customers/viewinvoice.php?customer_id=" . $arrScrape ['CustomerId']
		);
		...
		if ($strResponse <> "1")
		{
			...
			$arrReport ['<Response>'] = "MANAGE INVOICE PAGE HAS BEEN CACHED";
		}
		else
		{
			// Add something to the Report
			$arrReport ['<Response>'] = "FAILED TO RETRIEVE MANAGE INVOICE PAGE";
		}
