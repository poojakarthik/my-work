...
if (MDB2::isError($resBegin = $dbAdmin->beginTransaction()))
{
...
try
{
	...
	while ($arrDocumentContent = $resDocumentContent->fetchRow())
	{
		...
		if (MDB2::isError($strNewContent = $dbAdmin->quote($mixUncompressed, 'text')))
		{
	...
	if (MDB2::isError($resCommit = $dbAdmin->commit()))
	{
...
{
	if (MDB2::isError($resRollback = $dbAdmin->rollback()))
	{
