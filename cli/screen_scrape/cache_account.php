<?php
	...
	$rptReport = new Report (
		"+	ETECH CUSTOMER ACCOUNT DATABASE SETUP UTILITY: " . date ("Y-m-d h:i:s A"),
	...
	*/
	...
	$insScrape = new StatementInsert ('ScrapeAccount');
	...
	foreach ($cstCustomers->CustomerList () AS $intCustomerId)
	{
		...
		*/
		...
		$strResponse = $cnnConnection->Transmit (
			"GET",
			"https://sp.teleconsole.com.au/sp/customers/editdetails.php?customer_id=" . $intCustomerId
		);
		...
		*/
		...
		$rptReport->AddMessageVariables (
			...
			Array (
				...
				"<CustomerID>"		=> trim ($intCustomerId),
				"<Response>"		=> "ACCOUNT PAGE HAS BEEN SET UP"
			)
		);
