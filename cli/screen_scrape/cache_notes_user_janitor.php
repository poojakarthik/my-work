<?php
	...
	$rptReport = new Report (
		"+	ETECH USER NOTE JANITOR : " . date ("Y-m-d h:i:s A"),
	...
	$updScrape = new StatementUpdate ('ScrapeNoteUser', 'CustomerId = <CustomerId>', $arrScrapeSave);
	...
	while (true)
	{
		...
		$strResponse = $cnnConnection->Transmit (
			"GET",
			"https://sp.teleconsole.com.au/sp/customers/shownotes.php?customer_id=" . $arrScrape ['CustomerId']
		);
		...
		if ($strResponse <> "1")
		{
			...
			$arrReport ['<Response>'] = "USER NOTE HAS BEEN CACHED";
		}
		else
		{
			// Add something to the Report
			$arrReport ['<Response>'] = "FAILED TO RETRIEVE USER NOTE PAGE";
		}
