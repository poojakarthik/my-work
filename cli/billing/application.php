 ...
 */
 class ApplicationBilling extends ApplicationBaseClass
 {
	...
	function GenerateInvoices($arrAccounts, $bolReturnData = FALSE, $bolRegenerate = FALSE)
	{
		...
		foreach ($arrAccounts as $arrAccount)
		{
			...
			if (!$bolRegenerate)
			{
				...
				if($qryUpdateCDRs->Execute("UPDATE CDR JOIN Service ON Service.Id = CDR.Service SET InvoiceRun = '{$this->_strInvoiceRun}', CDR.Status = ".CDR_TEMP_INVOICE." WHERE CDR.Status = ".CDR_RATED." AND CDR.Account = {$arrAccount['Id']} AND CDR.Credit = 0 AND Service.Status IN (".SERVICE_ACTIVE.", ".SERVICE_DISCONNECTED.")") === FALSE)
				{
	 ...
	 */
 	...
 	{
 		$this->_rptBillingReport->AddMessage("[ EMAILING INVOICES ]\n");
 		...
 		while ($arrInvoice = $selInvoices->Fetch()) 
 		{
 			...
 			foreach ($arrDetails as $arrDetail)
 			{
				...
				$strContent	=	"Your {$arrCustomerGroups[$arrDetail['CustomerGroup']]['external_name']} Invoice, for Account Number {$arrInvoice['Account']}, is now ready for viewing via the online customer portal.  To access the portal please go to: {$arrCustomerGroups[$arrDetail['CustomerGroup']]['customer_exit_url']} and enter your username & password.\n\n" .
								"If you are yet to setup your customer account go to: {$arrCustomerGroups[$arrDetail['CustomerGroup']]['customer_exit_url']} and click on �Setup Account� and follow the prompts. Should you have any difficulties accessing the customer portal please email {$arrCustomerGroups[$arrDetail['CustomerGroup']]['outbound_email']}.\n\n" .
								"Regards,\n\n" .
