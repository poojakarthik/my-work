 ...
 */
 class ApplicationBilling extends ApplicationBaseClass
 {
	 ...
	 */
 	function __construct($arrConfig)
 	{
		...
		$this->selDebitsCredits				= new StatementSelect(	"Charge",
															 		...
															 		"Service = <Service> AND Status = ".CHARGE_TEMP_INVOICE." AND invoice_run_id = <invoice_run_id>",
															  		NULL,
															  		"2",
															  		"Nature");
		...
		$this->selAccountDebitsCredits		= new StatementSelect(	"Charge",
																 	"Nature, SUM(Amount) AS Amount",
															 		"Account = <Account> AND ISNULL(Service) AND Status = ".CHARGE_TEMP_INVOICE." AND invoice_run_id = <invoice_run_id>",
															  		NULL,
															  		"2",
															  		"Nature");
		...
		$this->arrCDRCols['invoice_run_id']	= NULL;
		...
		$this->arrInvoiceData 	['invoice_run_id']	= NULL;
	 ...
	 *
	 * Handles the generation of a Temporary Invoice Run
	 *
	 ...
	 * @method
	 */
 	function Execute($intInvoiceRunType)
 	{		
		// Start the stopwatch
		$this->Framework->StartWatch();

		// If there is a Temporary Invoice Run, then Revoke it
		if (!$this->Revoke())
		{
			// Failed, throw an exception
			throw new Exception("Revoke() failed: aborting Execute()");
		}
		
		//  Save the current time to be used as the "Bill Time"
		$this->_intBillDatetime			= time();
		
		// Create InvoiceRun entry & Bill Time
		$arrInvoiceRun						= Array();
		$arrInvoiceRun['InvoiceRun']		= $strInvoiceRun = date("YmdHis", $this->_intBillDatetime);
		$arrInvoiceRun['BillingDate']		= date("Y-m-d", $this->_intBillDatetime);
		$arrInvoiceRun['invoice_run_type']	= $intInvoiceRunType;
		$insInvoiceRun						= new StatementInsert("InvoiceRun", $arrInvoiceRun);
		if (!$insInvoiceRun->Execute($arrInvoiceRun))
		{
			if ($insInvoiceRun->Error())
			{
				// DB Error
				return Array();
			}
	...
	{
		$strBillDate					= date("Y-m-d", $this->_intBillDatetime);
		$strBillDatetime				= date("Y-m-d H:i:s", $this->_intBillDatetime);
		...
		$arrUpdateData['invoice_run_id']	= NULL;
		...
		foreach ($arrAccounts as $arrAccount)
		{
			...
			if (!$bolRegenerate)
			{
				...
				if($qryUpdateCDRs->Execute("UPDATE CDR USE INDEX (Account_2) JOIN Service ON Service.Id = CDR.Service SET invoice_run_id = {$intInvoiceRunId}, InvoiceRun = '{$this->_strInvoiceRun}', CDR.Status = ".CDR_TEMP_INVOICE." WHERE CDR.Status = ".CDR_RATED." AND CDR.Account = {$arrAccount['Id']} AND CDR.Credit = 0 AND Service.Status IN (".SERVICE_ACTIVE.", ".SERVICE_DISCONNECTED.")") === FALSE)
				{
