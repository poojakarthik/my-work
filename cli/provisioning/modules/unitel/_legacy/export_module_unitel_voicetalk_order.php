...
//----------------------------------------------------------------------------//
// module_export_unitel_voicetalk_order
//----------------------------------------------------------------------------//
/**
 * module_export_unitel_voicetalk_order
 *
 * Unitel Export Module for the provisioning engine (Daily Order File) for VoiceTalk
 *
 * Unitel Export Module for the provisioning engine (Daily Order File) for VoiceTalk
 *
 * @file		module_export_unitel_voicetalk_order.php
 * @language	PHP
...
//----------------------------------------------------------------------------//
// ProvisioningModuleExportUnitelVoiceTalkOrder
//----------------------------------------------------------------------------//
/**
 * ProvisioningModuleExportUnitelVoiceTalkOrder
 *
 ...
 *
 * @package		provisioning
 * @class		ProvisioningModuleExportUnitelVoiceTalkOrder
 */
 class ProvisioningModuleExportUnitelVoiceTalkOrder extends ProvisioningModuleExport
 {
	//------------------------------------------------------------------------//
	// __construct()
	//------------------------------------------------------------------------//
	 ...
	 * __construct()
	 *
	 * Constructor method for ProvisioningModuleExportUnitelVoiceTalkOrder
	 *
	 * Constructor method for ProvisioningModuleExportUnitelVoiceTalkOrder
	 *
	 * @return		ProvisioningModuleExportUnitelVoiceTalkOrder
	 *
	 * @method
	 */
 	function  __construct($ptrDB)
 	{
		...
		$selLastGenerated = new StatementSelect("Config", "Value", "Application = 4 AND Module = 'UnitelVoiceTalk' AND Name = 'DailyOrderLastSent'");
 	...
 	function BuildRequest($arrRequest)
	{
		...
		$this->_selGetSequence->Execute(Array('Module' => "UnitelVoiceTalk", 'Name' => "FullServiceRecordSequence"));
 	...
 	function SendRequest()
	{
		...
		$this->_selGetSequence->Execute(Array('Module' => "UnitelVoiceTalk", 'Name' => "FullServiceFileSequence"));
		if(!($arrResult = $this->_selGetSequence->Fetch()))
		{
			// Missing config definitions
			return FALSE;
		...
		$this->_selGetFullServiceRequests->Execute(Array('Carrier' => CARRIER_UNITEL_VOICETALK));
		...
		if(file_exists(UNITEL_VOICETALK_LOCAL_DAILY_ORDER_DIR.$this->_strFile))
		{
			// Upload the Daily Order File
			ftp_chdir($resFTPConnection, UNITEL_REMOTE_DAILY_ORDER_DIR);
			ftp_put($resFTPConnection, $this->_strFile, UNITEL_VOICETALK_LOCAL_DAILY_ORDER_DIR.$this->_strFile, FTP_ASCII);
		}
	...
	/*	$this->_insProvisioningExport = new StatementInsert();
		$arrData = Array();
		$arrData['Location']	= UNITEL_VOICETALK_LOCAL_DAILY_ORDER_DIR.$this->_strFile;
		$arrData['Carrier']		= CARRIER_UNITEL_VOICETALK;
		$arrData['Status']		= */
		...
		$updLastGenerated = new StatementUpdate("Config", "Application = 4 AND Module = 'UnitelVoiceTalk' AND Name = 'DailyOrderLastSent'", $arrColumns);
