 ...
 * master_multitask
 *
 * Multitasking implementation of Master
 *
 * Multitasking implementation of Master
 *
 * @file		master_multitask.php
 * @language	PHP
 * @package		master
 ...
 */
 class ApplicationMasterMultitask extends ApplicationMaster
 {
 	//------------------------------------------------------------------------//
	// __construct
	//------------------------------------------------------------------------//
	...
	function Run()
	{
		...
		// Run
		$intTime = time();
		while (($intCurrentTime = time()) > $intTime)
		{
			// Update time
			$intTime = $intCurrentTime;
			
			// store current run id (not used in this version)
			$this->_intCurrentRun = 0;
	...
	function _Run()
	{
		...
		foreach ($this->_arrScript as $strScriptName=>$arrScript)
		{
			...
			if ($intTimeNow > (int)$arrScript['NextRun'] && !$arrScript['Instance'])
			{				
