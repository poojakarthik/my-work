...
abstract class NormalisationModule extends CarrierModule {
	...
	function Validate($bolAsArray=false) {
		...
		if ($this->_arrNormalisedData["Source"] != "") { // 3
			$arrValid['Source'] = preg_match("/^\d+$|^\+\d+$|^\d{5,}(X+|\d+| +|\d+REV)I?$/i", $this->_arrNormalisedData["Source"]);
		} else {
		...
		if ($this->_arrNormalisedData["Destination"] != "") { // 4
			$arrValid['Destination'] = preg_match("/^\d+$|^\+\d+$|^\d{5,}(X+|\d+| +|\d+REV)I?$/i", $this->_arrNormalisedData["Destination"]);
		} else {
